        ...
        if (typeofLastVal=='number'){
          ...
          if (expression.length>=1){
            if((expression.length==1)&&(expression[0]=='0')
              &&(button.typeVal=='number')&&(button.value!='.')){
            ...
            else copy_array[copy_array.length-1]=copy_array[copy_array.length-1]+button.value;
          }
        }
      ...
      if(typeofLastVal=='number'){
        if(expression.length==1){
          if(lastVal.length==1) copy_array=['0'];
            ...
            copy_array[copy_array.length-1]=copy_array[copy_array.length-1].slice(0, -1); 
          }
        }
      ...
      if(expression.length==1) return setExpression(expression);
        ...
        for(let i=0;i<expression.length;i++){
          ...
          if ((expression[i]=='*')||(expression[i]=='/')){
            if (expression[i]=='*') expression[i-1]=String(Number(expression[i-1])*Number(expression[i+1])); 
            else if (expression[i]=='/') expression[i-1]=String(Number(expression[i-1])/Number(expression[i+1]));
